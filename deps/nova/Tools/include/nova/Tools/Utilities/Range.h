...
class Range
{
  ...
  public:
    ...
    OutCode ComputeOutCode(TV point) const 
    {
        OutCode code=INSIDE;
    ...
    void Project(TV& point,TV& velocity) 
    {
        OutCode code=ComputeOutCode(point);
        if(code&LEFT) {
            point(0)=min_corner(0);
            if(velocity(0)<(T)0.) velocity(0)=(T)0.;}
        else if(code&RIGHT) {
        ...
        if(code&BOTTOM) {
            ...
            if(velocity(1)<(T)0.) velocity(1)=(T)0.;
        }
        else if(code&TOP) {
